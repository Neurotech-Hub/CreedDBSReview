...
Source hardware: `v8.1/MouseStim_v8.1.pdf`, `v8.1/MouseStim_v8.1.net`, `v8.1/BOM.csv`.

## 1. Measured system current

Battery basis: two size-13 (PR48) zinc-air cells in series, ~2.8 V nominal and ~280 mAh pack capacity (series cells add voltage, not capacity). Lifetime assumes 90% usable capacity = 252 mAh.

| Mode | Measured avg current | Lifetime to 90% battery | Notes |
|---|---:|---:|---|
| Sleep | 1.47 mA | 7.14 days | Likely not true BGM220S EM2/EM3 sleep. |
| BLE Advertising | 1.62 mA | 6.48 days | +0.15 mA over measured sleep. |
| BLE Connected | 1.89 mA | 5.56 days | +0.42 mA over measured sleep. |
| Stimulating, 60 µA | 3.15 mA | 3.33 days | +1.26 mA over BLE connected. |
| Stimulating, 400 µA | 3.42 mA | 3.07 days | +0.27 mA over 60 µA stimulation. |
...
```text
2x size-13 zinc-air cells in series (~2.8 V, ~280 mAh)
        |
        v
...
| Assumption | Value |
|---|---:|
| Battery bus voltage | 2.8 V nominal (2× zinc-air in series) |
| +5 V boost efficiency for estimates | 85% |
| Battery current for +5 V load | `I_batt ≈ I_5V × 5 / (2.8 × 0.85) ≈ 2.10 × I_5V` |
| Battery current for 3.3 V LDO load, via +5 V boost | `I_batt ≈ I_3V3 × 3.3 / (2.8 × 0.85) ≈ 1.39 × I_3V3` |
| Battery current for ±5 V op-amp load | `I_batt ≈ I_supply × 10 / (2.8 × 0.85) ≈ 4.20 × I_supply`, where `I_supply` is the op-amp supply current flowing across both rails. |
...
|---|---|---:|---|---:|---:|---|---|
| IC2 | BGM220SC12WGA2R | 3.3 V VDD | BLE + pulse timing/control | EM0 active: ~22-38 µA/MHz. At 38.4 MHz, ~0.84-1.46 mA before radio. EM1: ~13-17 µA/MHz, ~0.50-0.65 mA at 38.4 MHz. EM2: ~1.4 µA. RX active ~4.2 mA during packet activity. TX 0 dBm ~4.6 mA, 6 dBm ~8.8 mA during TX. | EM0 @38.4 MHz: ~1.2-2.0 mA battery. EM1 @38.4 MHz: ~0.70-0.90 mA battery. Radio bursts: ~5.8-12.2 mA battery while active. | Highest software target. Avoid busy loops. Use TIMER/PRS/PWM for pulse edges. Use LDMA/SPI for DAC updates. Drop HCLK when possible. Enter EM1 between pulse edges. Duty-cycle BLE. Avoid debugger/RTT/logging during current tests. | High |
...
| D1/R1 | LED + 1 kΩ | 3.3 V VDD | Status indicator | Off: ~0. On: roughly `(3.3 V - Vf) / 1 kΩ`, commonly ~1-2 mA. | On: ~1.4-2.8 mA battery | Ensure LED is disabled during chronic operation. Avoid heartbeat blink unless explicitly needed. | High if currently active |
...
| Ref | Part/value | Rail | Role | Expected component current | Battery-equivalent estimate | MCU/software leverage | Priority |
|---|---|---:|---|---:|---:|---|---|
| U1 | MAX17220ELT_T | battery -> +5 V | Always-on boost converter | Sub-µA quiescent class, but all +5 V and 3.3 V loads pass through this converter. | Load multiplier: +5 V loads cost ~2.10× at battery. 3.3 V loads cost ~1.39× at battery. | None if it must stay on for MCU VDD. Major architectural issue: MCU VDD depends on +5 V. | High architecture |
...
| U7 | MAX1853 | +5 V -> -5 V | Negative rail for analog stim | ~165 µA typical / 320 µA max quiescent at 25 °C, shutdown near nA/0.1 µA class. | Quiescent alone: ~0.35 mA battery typical, ~0.67 mA max. Additional -5 V loads increase this. | Disable when not stimulating. During chronic stimulation it is fixed overhead unless pulses can be grouped/duty-cycled. | High |
...
| Ref | Part/value | Rail | Role | Expected component current | Battery-equivalent estimate | MCU/software leverage | Priority |
|---|---|---:|---|---:|---:|---|---|
| U6 | LT6020IDD-1-PBF | +5 V / -5 V | Dual op amp, Howland pump | ~90 µA/amplifier typical, 100 µA/amplifier max. Dual: ~180-200 µA across a 10 V supply span. Shutdown ~1.4 µA typical. | Active quiescent: ~0.76-0.84 mA battery if held enabled. Shutdown: ~0.006 mA battery. Firmware already asserts SHDN per pulse; the on-window is ~150 µs settle + 30 µs + P + 5·P + 30 µs, i.e. ~10% duty at P = 90 µs and ~50% at P = 600 µs, so the averaged cost is ~0.08-0.42 mA battery depending on pulse width. | Per-pulse shutdown is already implemented. Remaining leverage is shortening the 150 µs settle window and avoiding the amplifier being enabled outside pulse windows. | High |
| U4 | AD5683BCPZ-RL7 (external-reference variant, no internal reference) | +5 V; VREF tied to VDDA (LT6656 1.25 V) | DAC amplitude command | Normal mode: ~110 µA typ / 180 µA max. Power-down: ~2 µA. | Normal: ~0.23-0.38 mA battery. Power-down: ~0.004 mA battery. | Update DAC only when amplitude changes. Consider power-down between trains if the 6 µs wake is acceptable. Use DMA/SPI rather than CPU polling. | High |
| U5 | LT6656-1.25 | +5 V | Precision 1.25 V DAC/reference (VDDA) | <1 µA supply current; can source load current into DAC VREF. AD5683 reference input current may be ~26 µA at gain=1 or ~47 µA at gain=2 (firmware uses gain=2). | LT6656 alone <0.002 mA battery. With DAC VREF load: ~0.06-0.10 mA battery. | Keep powered only when DAC/stim active if feasible. | Low-Medium |
...
| BT1/BT2 | size-13 (PR48) zinc-air cells, in series | Energy source | n/a | ~2.8 V, ~280 mAh pack (two ~280 mAh cells in series). |
...
| LT6020 dual op amp on ±5 V | ~0.08-0.42 mA averaged (upper bound ~0.8 mA if held enabled) | Medium | Firmware cycles SHDN per pulse; on-duty is ~10-50% depending on pulse width, so the continuous figure is an upper bound reached only at long pulse widths. |
| MAX1853 -5 V inverter quiescent | ~0.35-0.7 mA | High | Fixed while negative rail enabled (SHDN is tied to +5 V, so it is always on). |
| AD5683 DAC active | ~0.23-0.38 mA | High | External-reference part; never powered down by firmware. |
| LT6656 + DAC VREF input | ~0.06-0.1 mA | Medium | Reference itself is tiny; DAC VREF input current may dominate. |
...
Expected takeaway: the 3.42 mA active current is plausible if the MCU is held awake for the whole train, the ±5 V rails and DAC are continuously powered, the op amp is on for a substantial fraction of each period, and there is some additional DC current in the Howland network. With the op amp duty-cycled, a larger share than first assumed falls on the MCU and the Howland network, which makes the rail-current measurements in §7 more important, not less.
...
Expected benefit:
- Moving from EM0 busy-wait at 38.4 MHz to EM1 at 38.4 MHz may save roughly `(0.84-1.46 mA) - (0.50-0.65 mA)` at the BGM220S, equivalent to about `0.3-1.1 mA` battery current depending on clock and activity.
...
Actions:
- The populated part is the AD5683BCPZ-RL7 (no internal reference); VREF comes from the LT6656 via VDDA, so there is no reference to disable.
...
- Power down DAC between trains if stimulation is not truly continuous; power-down exit is ~6 µs on this part.
...
| U4/U5 DAC/reference supply | Confirms DAC supply current and VREF input current. |
...
- `v8.1/MouseStim_v8.1.pdf`, `v8.1/MouseStim_v8.1.net`, `v8.1/BOM.csv` in this repository.
